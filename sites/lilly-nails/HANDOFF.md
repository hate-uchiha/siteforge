...
- [ ] Business name, phone, email, address, opening hours in `clients/lilly-nails.json`, then rebuild
- [ ] Gallery tiles: real job photos in `clients/lilly-nails.json` then rebuild
...
1. Create a project on Cloudflare Pages or Netlify, drag the `sites/lilly-nails` folder in
2. Point the client's domain at it and switch the SSL on
3. Rebuild and re-upload whenever content changes: `node build.mjs lilly-nails`
