# Zee Barbers handoff notes

Built with SiteForge from the `barber` preset in demo mode.

## Replace before launch

- [ ] Business name, phone, email, address, opening hours in `clients/zee-barbers.json`, then rebuild
- [ ] Gallery tiles: real job photos in `clients/zee-barbers.json` then rebuild
...
1. Create a project on Cloudflare Pages or Netlify, drag the `sites/zee-barbers` folder in
2. Point the client's domain at it and switch the SSL on
3. Rebuild and re-upload whenever content changes: `node build.mjs zee-barbers`
